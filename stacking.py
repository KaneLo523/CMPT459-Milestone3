...
def stacking_classifier(train_df, test_df):
    ...
    for train_index, test_index in kf.split(X):
        ...
        y_pred = stacking.predict(X_validation)
        y_pred1 = stacking.predict_proba(X_validation)
        training_scores.append(stacking.score(X_train, y_train))
    ...
    submission.to_csv('initial_stacking_submission.csv', index=False)
...
def improved_stacking_classifier(train_df, test_df):
    ...
    estimators = [
     ...
     ('rf', RandomForestClassifier(n_estimators=1000, max_depth = 13))
    ]
    ...
    titles_columns=["listing_id","high","medium","low"]
    submission=submission.reindex(columns=titles_columns)
    submission.to_csv('improved_stacking_submission.csv', index=False)
...
def main():
    ...
    # Already performed parameter tuning
    # parameter_tuning(train_df) 

  
    stacking_classifier(train_df, test_df)
    print("-------------")
    improved_stacking_classifier(train_df, test_df)
